...
const Header = () => {
  ...
  return (
    ...
    <div className='sticky top-0 z-30 w-full bg-main backdrop-blur-3xl print:hidden'>
      <div className='m-auto flex h-16 w-[95%] items-center justify-between md:h-20 lg:w-[90%] x1:w-[1300px]'>
    <Link href={'/'}>
    <Image
    src={Logo}
    alt="Logo"    
...
/>                                                                                
  ...
  <div className='relative cursor-pointer' onClick={() => setshowMenu(!showMenu)}>
    ...
    {showMenu && (
    <div className='my absolute right-0 mt-2 w-48 rounded-lg bg-white text-zinc-800'>
    ...
    <div className='py-1 px-2 text-sm text-zinc-600 hover:text-main'>{val.text}</div>
    </Link>
    ))}
    <h1 className='mt-5 text-left text-lg font-bold text-zinc-600 border-t border-zinc-300 pt-5 pb-3'>
    Advanced Courses
    </h1>
    {tracks.map((val) => (
    <Link href={" /tracks/${val.id}/4"} key={val.id}>
    <div className='py-1 px-2 text-sm text-zinc-600 hover:text-main'>{val.name}</div>
    </Link>
  ))}
</div>
</div>
...
<h1 className='mt-3 px-2 text-lg font-bold text-zinc-800'>Core Courses</h1>
{CompulsoryData.map((val,index) => (
  ...
  <h1 className='mt-3 px-2 text-lg font-bold text-zinc-800 border-t border-zinc-30 pt-5'>Advanced Couses</h1>
